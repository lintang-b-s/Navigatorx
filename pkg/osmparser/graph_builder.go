...
func buildGraph[W util.RoutingNumber](
	...
	numV uint32,
	roadNetwork bool,
) (*da.Graph, *costfunction.TimeFunction[W], [][]da.Index) {
	var (
	...
	addParallelVIaEdges := func(wayId int64, way osmWay) {
	...
	addTwoWayTurnCost := func(wayId int64, way osmWay) {
		/*
	...
	addViaNodeTurnRestriction := func(wayId int64, way osmWay, fromNodes []da.Index, restriction restriction, fromResId int) {
		/*
	...
	for wayId, way := range p.ways {
		...
		for fromResId, restriction := range fromRestrictions {
			...
			if !restriction.isWay {
				addViaNodeTurnRestriction(wayId, way, fromNodes, restriction, fromResId)
			} else if restriction.isWay {
				addViaWayTurnRestriction(wayId, way, fromNodes, restriction, fromResId)
			}
		}
	}
