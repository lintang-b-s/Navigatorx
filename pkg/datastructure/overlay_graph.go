...
func (og *OverlayGraph) WriteToFile(filename string) error {
	...
	if err != nil {
		return fmt.Errorf("WriteToFile: failed to create file: %s: %w", filename, err)
	}
	defer f.Close()

	snp := s2.NewWriter(f)
	if err != nil {
		return fmt.Errorf("WriteToFile: failed to create s2 writer: %w", err)
	}
	defer snp.Close()
	...
	for i := 0; i < len(offsets); i++ {
		if i > 0 {
			if _, err = fmt.Fprintf(w, " "); err != nil {
				return fmt.Errorf("WriteToFile: failed to write offset separator at index %d: %w", i, err)
			}
		}
		if _, err = fmt.Fprintf(w, "%d", offsets[i]); err != nil {
			return fmt.Errorf("WriteToFile: failed to write offset[%d]: %w", i, err)
		}
	}

	if _, err = fmt.Fprintf(w, "\n"); err != nil {
		return fmt.Errorf("WriteToFile: failed to write newline after offsets: %w", err)
	}

	for l := 1; l <= og.levelInfo.GetLevelCount(); l++ {
		if l > 1 {
			if _, err = fmt.Fprintf(w, " "); err != nil {
				return fmt.Errorf("WriteToFile: failed to write separator before level %d vertex count: %w", l, err)
			}
		}
		if _, err = fmt.Fprintf(w, "%d", og.NumberOfVerticesInLevel(l)); err != nil {
			return fmt.Errorf("WriteToFile: failed to write number of vertices in level %d: %w", l, err)
		}
	}

	if _, err = fmt.Fprintf(w, "\n"); err != nil {
		return fmt.Errorf("WriteToFile: failed to write newline after level vertex counts: %w", err)
	}

	if _, err = fmt.Fprintf(w, "%d \n", len(og.overlayVertices)); err != nil {
		return fmt.Errorf("WriteToFile: failed to write overlay vertices count: %w", err)
	}
	...
	for i, vertex := range og.overlayVertices {
		if _, err = fmt.Fprintf(w, "%d %d %d %d", vertex.cellNumber, vertex.neighborOverlayVertex,
			...
			return fmt.Errorf("WriteToFile: failed to write overlayVertex[%d]: %w", i, err)
		}
		for j, e := range vertex.entryExitPoint {
			if _, err = fmt.Fprintf(w, " %d", e); err != nil {
				return fmt.Errorf("WriteToFile: failed to write overlayVertex[%d] entryExitPoint[%d]: %w", i, j, err)
			}
		}
		if _, err = fmt.Fprintf(w, "\n"); err != nil {
			return fmt.Errorf("WriteToFile: failed to write newline after overlayVertex[%d]: %w", i, err)
		}
	}

	if _, err = fmt.Fprintf(w, "%d\n", og.weightVectorSize); err != nil {
		return fmt.Errorf("WriteToFile: failed to write weightVectorSize: %w", err)
	}
	...
	for i, id := range overlayIdMapping {
		if i > 0 {
			if _, err = fmt.Fprintf(w, " "); err != nil {
				return fmt.Errorf("WriteToFile: failed to write separator before overlayIdMapping[%d]: %w", i, err)
			}
		}
		if _, err = fmt.Fprintf(w, "%d", id); err != nil {
			return fmt.Errorf("WriteToFile: failed to write overlayIdMapping[%d]: %w", i, err)
		}
	}

	if _, err = fmt.Fprintf(w, "\n"); err != nil {
		return fmt.Errorf("WriteToFile: failed to write newline after overlayIdMapping: %w", err)
	}

	for l := 1; l <= og.levelInfo.GetLevelCount(); l++ {
		if _, err = fmt.Fprintf(w, "%d\n", og.numberOfCellsInLevel(l)); err != nil {
			return fmt.Errorf("WriteToFile: failed to write numberOfCells in level %d: %w", l, err)
		}
		for cellNumber, cell := range og.cellMapping[l-1] {
			if _, err = fmt.Fprintf(w, "%d %d %d %d %d %d\n", cellNumber, cell.numEntryPoints, cell.numExitPoints,
				...
				return fmt.Errorf("WriteToFile: failed to write cell[%d] in level %d: %w", cellNumber, l, err)
			}
		}
	}

	if err = w.Flush(); err != nil {
		return fmt.Errorf("overlayGraph.WriteToFile: failed to flush bufio writer: %w", err)
	}
	return nil
}
...
func ReadOverlayGraph(filename string) (*OverlayGraph, error) {
	...
	for _, token := range tokens {
		...
		if err != nil {
			return nil, fmt.Errorf("ReadOvelayGraph: failed parseInt offset: %v: %w", token, err)
		}
	...
	for _, token := range tokens {
		...
		if err != nil {
			return nil, fmt.Errorf("ReadOvelayGraph: failed parseInt vertexCountInLevel: %v: %w", token, err)
		}
	...
	if err != nil {
		return nil, fmt.Errorf("ReadOvelayGraph: failed parseInt vertexCount: %v: %w", tokens[0], err)
	}
	...
	for i := Index(0); i < Index(vertexCount); i++ {
		...
		if err != nil {
			return nil, fmt.Errorf("ReadOvelayGraph: failed to parseInt vCellNumber: %v: %w", tokens[0], err)
		}
		vertex.cellNumber = Pv(vCellNumber)
		vneighbor, err := util.ParseInt(tokens[1])
		if err != nil {
			return nil, fmt.Errorf("ReadOvelayGraph: failed to parseInt vneighbor: %v: %w", tokens[1], err)
		}
		vertex.neighborOverlayVertex = Index(vneighbor)

		oriVertex, err := util.ParseInt(tokens[2])
		if err != nil {
			return nil, fmt.Errorf("ReadOvelayGraph: failed to parseInt oriVertex: %v: %w", tokens[2], err)
		}
		vertex.originalVertex = Index(oriVertex)

		oriEdge, err := util.ParseInt(tokens[3])
		if err != nil {
			return nil, fmt.Errorf("ReadOvelayGraph: failed to parseInt oriEdge: %v: %w", tokens[3], err)
		}
		...
		for j := 4; j < len(tokens); j++ {
			...
			if err != nil {
				return nil, fmt.Errorf("ReadOvelayGraph: failed to parseInt entryExitPoint: %v: %w", tokens[j], err)
			}
	...
	if err != nil {
		return nil, fmt.Errorf("ReadOvelayGraph: failed to parseInt weightVectorSize: %v: %w", line, err)
	}
	...
	for _, token := range tokens {
		...
		if err != nil {
			return nil, fmt.Errorf("ReadOvelayGraph: failed to parseInt overlayIdMapping: %v: %w", token, err)
		}
	...
	for i := 0; i < levelInfo.GetLevelCount(); i++ {
		...
		for j := 0; j < cellsInLevel; j++ {
			...
			if err != nil {
				return nil, fmt.Errorf("ReadOvelayGraph: failed to ReadLine cellNumber: %v: %w", tokens[0], err)
			}
			cellNumber := Pv(ccn)
			numEntryExitPoint, err := util.ParseInt(tokens[1])
			if err != nil {
				return nil, fmt.Errorf("ReadOvelayGraph: failed to ReadLine numEntryExitPoint: %v: %w", tokens[1], err)
			}
			cell.numEntryPoints = Index(numEntryExitPoint)
			numExitPoints, err := util.ParseInt(tokens[2])
			if err != nil {
				return nil, fmt.Errorf("ReadOvelayGraph: failed to ReadLine numExitPoints: %v: %w", tokens[2], err)
			}
			cell.numExitPoints = Index(numExitPoints)

			cellOffset, err := util.ParseInt(tokens[3])
			if err != nil {
				return nil, fmt.Errorf("ReadOvelayGraph: failed to ReadLine cellOffset: %v: %w", tokens[3], err)
			}
			cell.cellOffset = Index(cellOffset)

			overlayIdOffset, err := util.ParseInt(tokens[4])
			if err != nil {
				return nil, fmt.Errorf("ReadOvelayGraph: failed to ReadLine overlayIdOffset: %v: %w", tokens[4], err)
			}
			cell.overlayIdOffset = Index(overlayIdOffset)

			numOverlayVertices, err := util.ParseInt(tokens[5])
			if err != nil {
				return nil, fmt.Errorf("ReadOvelayGraph: failed to ReadLine numOfOverlayVertices: %v: %w", tokens[5], err)
			}
