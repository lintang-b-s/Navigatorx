...
type MockAlternativeRouteAlgorithm struct {
	mock.Mock
}

func (m *MockAlternativeRouteAlgorithm) FindAlternativeRoutes(sp, tp da.PhantomNode, k int, reroute bool, startEdgeId da.Index) ([]routing.AlternativeRoute, float64, int64) {
	args := m.Called(sp, tp, k, reroute, startEdgeId)
	var r0 []routing.AlternativeRoute
	if args.Get(0) != nil {
		r0 = args.Get(0).([]routing.AlternativeRoute)
	}
	return r0, args.Get(1).(float64), args.Get(2).(int64)
}
...
func (m *MockSpatialIndex) SearchWithinRadius(qLat, qLon, radius float64, mode uint8) []da.Index {
	args := m.Called(qLat, qLon, radius, mode)
	var r0 []da.Index
	if args.Get(0) != nil {
		r0 = args.Get(0).([]da.Index)
	}
	return r0
}

type MockOnlineMapMatcherEngine struct {
	mock.Mock
}

func (m *MockOnlineMapMatcherEngine) OnlineMapMatch(gps *da.GPSPoint, k int, candidates []*ma.Candidate, speedMeanK, speedStdK, lastBearing float64) (*da.MatchedGPSPoint, []*ma.Candidate, float64, float64) {
	args := m.Called(gps, k, candidates, speedMeanK, speedStdK, lastBearing)
	...
	return r0, r1, args.Get(2).(float64), args.Get(3).(float64)
}
