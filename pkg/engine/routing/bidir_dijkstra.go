...
func (bs *BidirectionalDijkstra[W]) ShortestPathSearch(sp, tp da.PhantomNode) (W, float64, []da.Coordinate,
	...
	if tail != midHead {
		...
		totalDistance += bs.engine.GetSegmentLength(midOutEdge, true)
	}
	...
	for curInfo.GetParent().GetEdge() != sForwardId {
		...
		util.ReverseG(revGeom)
		finalPath = append(finalPath, revGeom...)
		totalDistance += bs.engine.GetSegmentLength(outEdge, true)
	...
	if tail != midOutEdgeHead {
		...
		totalDistance += bs.engine.GetSegmentLength(midOutEdgeId, true)
	}
	...
	for curInfo.GetParent().GetEdge() != tBackwardId {
		...
		finalPath = append(finalPath, geom...)
		totalDistance += bs.engine.GetSegmentLength(outEdge.GetEdgeId(), true)
...
func (bs *BidirectionalDijkstra[W]) forwardGraphSearch(uItem da.CRPQueryKey, source, target da.Index) {
	...
	bs.engine.graph.ForOutEdgesOf(uId, uEntryPoint, func(eId, head da.Index, exitPoint, entryPoint, turnTableId da.Index, turnType pkg.TurnType,
		...
		bs.engine.graph.ForOutEdgesOf(vId, entryPoint, func(_, _ da.Index, _, _, turnTableId2 da.Index, turnType2 pkg.TurnType,
			...
			if scannedByBackwardSearch && util.Lt(newVEntryIdTravelTime+bs.engine.metrics.GetTurnCost(turnTableId2)+
				vExitIdTravelTime, bs.shortestTravelTime) {

				bs.shortestTravelTime = newVEntryIdTravelTime + bs.engine.metrics.GetTurnCost(turnTableId2) +
					vExitIdTravelTime
...
func (bs *BidirectionalDijkstra[W]) backwardGraphSearch(uItem da.CRPQueryKey, source, target da.Index) {
	...
	bs.engine.graph.ForInEdgesOf(uId, uExitPoint, func(eId, tail da.Index, exitPoint, entryPoint, turnTableId da.Index,
		...
		bs.engine.graph.ForInEdgesOf(vId, exitPoint, func(_, _ da.Index, _, _, turnTableId2 da.Index,
			...
			if scannedByForwardSearch && util.Lt(vEntryIdTravelTime+bs.engine.metrics.GetTurnCost(turnTableId2)+
				newVExitIdTravelTime, bs.shortestTravelTime) {

				bs.shortestTravelTime = vEntryIdTravelTime + bs.engine.metrics.GetTurnCost(turnTableId2) +
