...
func (bs *CRPBidirectionalSearch) ShortestPathSearch(asId, atId datastructure.Index) (float64, float64, []datastructure.Coordinate,
	...
	unpacker := NewPathUnpacker(bs.engine.graph, bs.engine.overlayGraph, bs.engine.metrics, bs.engine.puCache, bs)
				...
				bs.engine.graph.ForOutEdgesOf(vId, datastructure.Index(outArc.GetEntryPoint()), func(e2 *datastructure.OutEdge,
					...
					if visitedByBackwardSearch {
						...
						if newPathTravelTime < bs.shortestTimeTravel {
							...
							vEntryIdVia := bs.adjustForwardOffBit(vEntryId)
							vExitIdVia := bs.adjustBackwardOffbit(vExitId)
							bs.viaVertices = append(bs.viaVertices, datastructure.NewViaVertex(vId, vEntryIdVia, vExitIdVia, vId))
						}
					}
					vExitId++
				})
					...
					bs.engine.graph.ForOutEdgesOf(originalW, datastructure.Index(outEdge.GetEntryPoint()), func(e *datastructure.OutEdge, exitPoint datastructure.Index, turn pkg.TurnType) {
						...
						if wVisitedByBackwardSearch {
							...
							if newPathTravelTime < bs.shortestTimeTravel {
								...
								wEntryIdVia := bs.adjustForwardOffBit(wEntryId)
								wExitIdvia := bs.adjustBackwardOffbit(wExitId)
								bs.viaVertices = append(bs.viaVertices, datastructure.NewViaVertex(originalW, wEntryIdVia, wExitIdvia, originalW))
							}
						}
						wExitId++
					})
