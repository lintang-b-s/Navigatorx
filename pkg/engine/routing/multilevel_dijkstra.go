...
func (bs *CRPBidirectionalSearch) ShortestPathSearch(sp, tp da.PhantomNode) (float64, []da.Index, bool) {
	...
	addViaVertex := func(uItem da.CRPQueryKey, forward bool, overlay bool) {
		if !overlay && forward {
			...
			bs.engine.graph.ForInEdgesOf(uId, uExitPoint, func(_, _ da.Index, _, _ float64, _, _, turnTableId da.Index,
...
func (bs *CRPBidirectionalSearch) forwardGraphSearch(uItem da.CRPQueryKey, source, target da.Index) {
	...
	bs.engine.graph.ForOutEdgesOf(uId, uEntryPoint, func(eId, head da.Index, weight, length float64, exitPoint, entryPoint, turnTableId da.Index, turnType pkg.TurnType,
		...
		if vQueryLevel == 0 {
			...
			vAlreadyLabelled := util.Lt(oldOverlayVIdTravelTime, pkg.INF_WEIGHT)
	...
	bs.engine.overlayGraph.ForOutNeighborsOf(u, uQueryLevel, func(v da.Index, wOffset da.Index) {
		...
		if !vAlreadyLabelled || (vAlreadyLabelled && util.Lt(newTravelTime, oldOverlayVIdTravelTime)) {
			...
			if wQueryLevel == 0 {
				...
				wAlreadyLabelled := util.Lt(oldWEntryIdTravelTime, pkg.INF_WEIGHT)
				...
				wExitId := exitOffset
				bs.engine.graph.ForOutEdgesOf(originalWId, outEdgeEntryPoint, func(_, _ da.Index, _, _ float64, _, _, turnTableId da.Index, turn pkg.TurnType,
					_ pkg.OsmHighwayType) {
				...
				wAlreadyLabelled := util.Lt(oldOverlayWIdTravelTime, pkg.INF_WEIGHT)
	...
	bs.engine.overlayGraph.ForInNeighborsOf(u, int(uQueryLevel), func(v da.Index,
		...
		if !vAlreadyLabelled || (vAlreadyLabelled && util.Lt(newTravelTime, oldOverlayVIdTravelTime)) {
			...
			if wQueryLevel == 0 {
				...
				wAlreadyLabelled := util.Lt(oldWExitIdTravelTime, pkg.INF_WEIGHT)
