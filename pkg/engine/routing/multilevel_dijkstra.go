...
func (bs *CRPBidirectionalSearch) ShortestPathSearch(sp, tp da.PhantomNode) (float64, []da.Index, bool) {
	...
	addViaVertex := func(uItem da.CRPQueryKey, forward bool, overlay bool) {
		if !overlay && forward {
			...
			uExitId := exitOffset
			bs.engine.graph.ForOutEdgesOf(uId, uEntryPoint, func(_, _ da.Index, _, _ float64, _, _ da.Index, _ pkg.TurnType,
			...
			bs.engine.graph.ForInEdgesOf(uId, uExitPoint, func(_, _ da.Index, _, _ float64, _, _ da.Index,
